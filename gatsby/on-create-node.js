  ...
  if (node.internal.type === 'MarkdownRemark') {
    ...
    switch (type) {
      case 'content':
        ...
        break;
      case 'lessons':
        const basename = path.dirname(node.fileAbsolutePath).split(path.sep).pop();
        const section = basename.replace(/^\d+-/, '');
        createNodeField({ node, name: 'type', value: 'lesson' });
        createNodeField({ node, name: 'slug', value: ['lessons', section, node.frontmatter.title].map(part => slugify(part)).join('/') });
